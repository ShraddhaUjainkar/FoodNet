...
import { INGREDIENT_CATALOG, INGREDIENT_ALIASES, ADDITIVES_CATALOG, } from '../src/services/catalog.service.js';
dotenv.config();
const pool = new pg.Pool({ connectionString: process.env.DATABASE_URL });
const adapter = new PrismaPg(pool);
const prisma = new PrismaClient({ adapter });
function mapConcernLevel(rating) {
    switch (rating) {
        case 'safe':
            return 'low';
        case 'caution':
...
async function main() {
    ...
    for (const [alias, standardName] of Object.entries(INGREDIENT_ALIASES)) {
        ...
        if (!ingredientId) {
            console.warn(`⚠ Skipping alias "${alias}" because "${standardName}" was not found`);
            continue;
        }
