  ...
  try {
    ...
    if (image) {
      ...
      if (!match) {
        res.status(400).json({
          ...
          error: {
            ...
            message: 'Invalid image format. Use data:image/png|jpeg|jpg|webp;base64,...',
          },
        });
        return;
      }
      ...
      try {
        ...
        logger.error({ err: error }, 'Image upload or analysis job enqueue failed');
    ...
    if (!rawText) {
      ...
      console.log(`Step 2 (Sync): OCR complete. Extracted text length: ${rawText.length}`);
    } else {
      console.log(`Step 2 (Sync): Using provided raw text. Length: ${rawText.length}`);
    }
    ...
    console.log(`Step 3 (Sync): Normalization complete. Matched: ${matchedIngredients.length} ingredients`);
