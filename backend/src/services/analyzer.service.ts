import { ADDITIVES_CATALOG } from './catalog.service.js';
import { NormalizedIngredient, MatchedIngredient } from './normalizer.service.js';
...
export function calculateHealthScore(ingredients: MatchedIngredient[]) {
  ...
  ingredients.forEach((ing) => {
    ...
    if (
      ...
      nameLower.includes('lactose') ||
      nameLower.includes('whey') ||
      nameLower.includes('casein')
    ) {
      allergens.push('Milk');
    }
    
    // Soy check
    if (nameLower.includes('soy') || nameLower.includes('lecithin')) {
      allergens.push('Soy');
    }
    ...
    if (
      ...
      allergens.push('Gluten (Wheat/Barley/Rye)');
    }
    ...
    // Sesame check
    if (nameLower.includes('sesame')) {
      allergens.push('Sesame Seeds');
    }
