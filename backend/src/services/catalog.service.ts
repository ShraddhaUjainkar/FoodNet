...
let cachedIngredientsMap: Map<string, IngredientCacheItem> | null = null;
let cachedAliasesMap: Map<string, IngredientCacheItem> | null = null;
let cachedAllIngredients: any[] | null = null;
let cachedAllAliases: any[] | null = null;

async function populateCache() {
  if (cachedIngredientsMap) return;
  ...
  cachedAllIngredients = allIngredients;

  for (const ing of allIngredients) {
    cachedIngredientsMap.set(ing.normalizedName, {
  ...
  cachedAliasesMap = new Map();
  cachedAllAliases = allAliases;

  for (const aliasItem of allAliases) {
    cachedAliasesMap.set(aliasItem.alias, {
...
export async function findIngredient(name: string): Promise<{
  ...
  const directMatch = cachedIngredientsMap!.get(cleanName);
  if (directMatch) {
    return directMatch;
  }

  const aliasMatch = cachedAliasesMap!.get(cleanName);
  if (aliasMatch) {
    return aliasMatch;
  }

  for (const aliasItem of cachedAllAliases!) {
  ...
  for (const ing of cachedAllIngredients!) {
